...
import SasaranOpdTable from './SasaranOpdTable';
import { getPeriodeList, getSasaranOpd } from '../services';
import type { Periode, SasaranOpdRow } from '../types';

function SasaranOpdContent() {
  ...
  const [rows, setRows] = useState<SasaranOpdRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const { rows: flatRows, tahunList: years } = await getSasaranOpd(kodeOpd!, tahun_awal, tahun_akhir, jenis_periode);
        setRows(flatRows);
        setTahunList(years);
      } catch {
        toast.error('Gagal memuat data sasaran OPD');
        setRows([]);
        setTahunList([]);
      } finally {
        setLoading(false);
  ...
  const handleActivate = (_newTahun: string, newKodeOpd: string) => {
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20">
                <div className="flex flex-col items-center gap-3 text-muted-foreground">
                  ...
                  <p className="text-sm">Memuat data sasaran OPD...</p>
                </div>
              </div>
            ) : rows.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <Building2 className="mb-4 size-14 text-muted-foreground/30" />
                <p className="text-sm text-muted-foreground">Tidak ada data sasaran OPD.</p>
              </div>
            ) : (
              <SasaranOpdTable
                rows={rows}
...
export default function SasaranOpdClient() {
  return (
    <Suspense
      fallback={
