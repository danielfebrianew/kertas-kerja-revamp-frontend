...
interface IndikatorState {
  ...
  sumber_data: string;
  targets: IndikatorTarget[];
}

interface TujuanOpdEditModalProps {
  idTujuanOpd: number;
  kodeOpd: string;
  onCancel: () => void;
  onSuccess: () => void;
...
export const TujuanOpdEditModal: React.FC<TujuanOpdEditModalProps> = ({
  idTujuanOpd,
  kodeOpd,
  onCancel,
  onSuccess,
  ...
  useEffect(() => {
    async function fetchData() {
      setLoadingData(true);
      try {
        const [detailRes, bidangRes] = await Promise.all([
          fetchApi<{ data: any }>({
            type: 'auth',
            method: 'GET',
            url: `/tujuan_opd/findone/${idTujuanOpd}`,
          }),
          fetchApi<{ data: BidangUrusan[] | { data: BidangUrusan[] } }>({
            type: 'auth',
            method: 'GET',
            url: `/bidang_urusan/findall/${kodeOpd}`,
          }),
        ]);

        const detail = detailRes.data?.data;
        const bidangRaw = bidangRes.data?.data;
        const bidangList = Array.isArray(bidangRaw) ? bidangRaw : [];
        setBidangUrusanList(bidangList);

        if (detail) {
          setTujuan(detail.tujuan ?? '');
          setKodeBidangUrusan(detail.kode_bidang_urusan ?? '');

          const tahunSet = new Set<string>();
          const indList: IndikatorState[] = (detail.indikator ?? []).map((ind: any) => {
            const targets: IndikatorTarget[] = (ind.target ?? []).map((t: any) => {
              tahunSet.add(t.tahun);
              return { id: t.id, tahun: t.tahun, target: t.target ?? '', satuan: t.satuan ?? '' };
            });
            return {
              id: ind.id,
              nama_indikator: ind.nama_indikator ?? ind.indikator ?? '',
              rumus_perhitungan: ind.rumus_perhitungan ?? '',
              sumber_data: ind.sumber_data ?? '',
              targets,
            };
          });
          setIndikators(indList);
  ...
  const addIndikator = () => {
    setIndikators([
      ...indikators,
      {
        ...
        rumus_perhitungan: '',
        sumber_data: '',
        targets: tahunList.map((tahun) => ({ tahun, target: '', satuan: '' })),
      },
    ]);
  };
  ...
  const handleIndikatorChange = (idx: number, field: keyof Omit<IndikatorState, 'targets' | 'id'>, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetchApi({
        type: 'auth',
        url: `/tujuan_opd/update/${idTujuanOpd}`,
        method: 'PUT',
        body: payload,
      });
      toast.success('Tujuan OPD berhasil diperbarui');
      onSuccess();
    } catch (error) {
