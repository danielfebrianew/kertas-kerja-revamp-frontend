// app/(authenticated)/data-master/master-opd/_components/modals/AddMasterOpd.tsx
...
interface CreateOpdResponse {
  code: number;
  status: string;
  data: any;
}

interface AddModalMasterOpdProps {
  onCancel: () => void;
  onSuccess: () => void;
}
...
export default function AddModalMasterOpd({
  onCancel,
  onSuccess,
}: AddModalMasterOpdProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status >= 400) {
        throw new Error(response.message || 'Gagal menambahkan Master OPD');
      }

      toast.success('Master OPD berhasil ditambahkan');
      setForm(initialForm);
      onSuccess();
    } catch (error) {
      const message =
